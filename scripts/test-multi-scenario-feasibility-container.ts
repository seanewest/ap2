...
} from "../src/api/multi-scenario-feasibility.fixtures.ts";

const ISSUER = "https://container-fixture.example/feasibility/v2.0";
const AUDIENCE = "api://batch-feasibility-container-fixture";
const KEY_ID = "batch-feasibility-container-fixture-key";
const image = `ap2-batch-feasibility:${process.pid}`;
const container = `ap2-batch-feasibility-${process.pid}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/multi-scenario-feasibility`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify(feasibleBatchRequest(true)),
      },
    );
    if (response.status !== 200) {
      throw new Error(
        `Container feasibility request returned HTTP ${response.status}.`,
      );
    }
    const result = await response.json() as Record<string, unknown>;
    if (
      result.label !== "FEASIBILITY_ONLY" ||
      result.status !== "feasible" ||
      result.planCount !== 2 ||
      !Array.isArray(result.blockers) ||
      result.blockers.length !== 0
    ) {
      throw new Error("Container returned an invalid feasibility summary.");
    }
    ...
    console.log("Production container signed-auth feasibility proof passed");
  } finally {
    if (containerCreated) {
