...
const ISSUER = "https://container-fixture.example/private-rehearsal/v2.0";
const AUDIENCE = "api://private-rehearsal-container-fixture";
const KEY_ID = "private-rehearsal-container-fixture-key";
const image = `ap2-private-rehearsal:${process.pid}`;
const container = `ap2-private-rehearsal-${process.pid}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/private-document-rehearsal-verification`,
      {
        method: "POST",
        headers: {
    ...
    if (
      ...
      result.scenarioId !== "private-document-evidence" ||
      result.syntheticBranch !== "cleaned-canary" ||
      result.externalEvidence !== "all-uninspected" ||
      result.claimCount !== 18
    ) {
    ...
    console.log(
      "Production container signed-auth private rehearsal proof passed",
    );
  } finally {
    if (containerCreated) {
