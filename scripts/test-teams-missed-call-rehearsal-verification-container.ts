...
const ISSUER = "https://container-fixture.example/teams-rehearsal/v2.0";
const AUDIENCE = "api://teams-rehearsal-container-fixture";
const KEY_ID = "teams-rehearsal-container-fixture-key";
const image = `ap2-teams-rehearsal:${process.pid}`;
const container = `ap2-teams-rehearsal-${process.pid}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/teams-missed-call-rehearsal-verification`,
      {
        method: "POST",
        headers: {
    ...
    if (
      ...
      result.scenarioId !== "teams-missed-call-observation" ||
      result.syntheticBranch !== "native-cleaned" ||
      result.cleanup !== "two-surface-absent" ||
      result.externalEvidence !== "all-uninspected" ||
      result.canonicalLearnerInterpretation !== "uninspected" ||
      result.claimCount !== 14
    ) {
    ...
    console.log(
      "Production container signed-auth Teams rehearsal proof passed",
    );
  } finally {
    if (containerCreated) {
