...
const ISSUER = "https://container-fixture.example/purview/v2.0";
const AUDIENCE = "api://purview-container-fixture";
const KEY_ID = "purview-container-fixture-key";
const image = `purview-rehearsal-verification:${process.pid}`;
const container = `purview-rehearsal-verification-${process.pid}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      result.scenarioId !== "purview-sharepoint-audit-boundary" ||
      result.syntheticContract !==
        "deduplicated-producer-attribution-terminal-verified" ||
      result.externalEvidence !== "all-uninspected" ||
      result.claimCount !== 14 ||
      result.producerAttributionClaimCount !== 1
    ) {
    ...
    console.log("Production container signed-auth Purview proof passed");
  } finally {
    if (containerCreated) {
...
async function waitForHealthy(baseUrl: string): Promise<void> {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    try {
      ...
      // The isolated local container may still be starting.
    }
