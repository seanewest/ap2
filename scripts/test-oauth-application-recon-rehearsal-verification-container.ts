...
const ISSUER = "https://container-fixture.example/oauth-recon/v2.0";
const AUDIENCE = "api://oauth-recon-container-fixture";
const KEY_ID = "oauth-recon-container-fixture-key";
const image = `ap2-oauth-recon-verification:${process.pid}`;
const container = `ap2-oauth-recon-verification-${process.pid}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      result.scenarioId !== "oauth-application-reconnaissance" ||
      result.fakeContract !== "ordered-four-read-terminal-verified" ||
      result.externalEvidence !== "all-uninspected" ||
      result.claimCount !== 13
    ) {
    ...
    console.log("Production container signed-auth OAuth recon proof passed");
  } finally {
    if (containerCreated) {
