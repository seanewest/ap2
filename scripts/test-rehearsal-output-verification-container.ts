...
const verifierModuleUrl =
  new URL("./verify-avd-three-vm-rehearsal-output.ts", import.meta.url).href;
const { canonicalAvdThreeVmRehearsalOutput } = (
  await import(verifierModuleUrl)
) as typeof import("./verify-avd-three-vm-rehearsal-output.ts");

const ISSUER = "https://container-fixture.example/rehearsal/v2.0";
const AUDIENCE = "api://rehearsal-container-fixture";
const KEY_ID = "rehearsal-container-fixture-key";
const image = `ap2-rehearsal-verification:${process.pid}`;
const container = `ap2-rehearsal-verification-${process.pid}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/rehearsal-output-verification`,
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify(canonicalAvdThreeVmRehearsalOutput()),
      },
    );
    if (response.status !== 200) {
      throw new Error(
        `Container verification returned HTTP ${response.status}.`,
      );
    }
    ...
    if (
      ...
      result.observations !== "synthetic-only" ||
      result.evidenceClaims !== "all-uninspected"
    ) {
      throw new Error("Container returned an invalid safe summary.");
    }
    ...
    console.log(
      "Production container signed-auth rehearsal verification passed",
    );
  } finally {
    if (containerCreated) {
      spawnSync("podman", ["rm", "--force", container], {
        encoding: "utf8",
      });
    }
