...
| Identity risk telemetry | Keep Microsoft Entra ID Protection and Unified risk telemetry available for the simulated users, including read-only native `riskyUsers` and `riskDetections` access through the protected development diagnostic identity. Do not use standing Conditional Access user-risk or sign-in-risk conditions, Microsoft-managed risk enforcement, or either legacy Identity Protection risk-policy switch to block or remediate simulated users automatically. An active risk record is telemetry/scenario evidence and is not baseline drift to dismiss. Microsoft service-level high-confidence blocking and limited threat-informed remediation remain platform behavior rather than tenant-configured policy. |
...
| Global Secure Access | Where licensed for the installation, retained endpoints can carry the supported GSA client and normal Internet traffic acquisition as standing infrastructure. Follow ordinary enterprise administration: deploy the Windows client as a required system-context Intune application to the exact retained managed-endpoint device cohort, and assign enabled Microsoft 365 and Internet traffic-forwarding profiles through an exact direct-member retained managed-user group rather than proof-only direct-user/manual assignments. Hide generated forwarding-profile enterprise applications from My Apps because they are acquisition/control objects rather than learner applications. Keep narrow development controls such as Rachel's TLS-inspection profile separately scoped when intentionally exceptional. This provides attributable corporate network/SWG telemetry in addition to MDE. |
