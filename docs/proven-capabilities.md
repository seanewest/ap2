...
| OneDrive read-only share | The hosted path created the exact 58-byte Homer file, configured Marge's direct read permission, observed Microsoft's still-propagating access state, then revoked the permission and removed the active file. | c91 delegated — Homer; c91 delegated — Marge for read-only verification | Active path absent and no Marge share remained; deleted-item/recycle-bin history retained. Marge opening and byte-verifying the file was **not** proven within the bounded wait |
