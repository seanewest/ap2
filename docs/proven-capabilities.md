...
| Standing Windows Hello-backed Entra passkeys | On 2026-08-22 AP2 added profile `AP2 Windows Hello passkeys` to the enabled Passkey/FIDO2 authentication-method policy and assigned it to `all_users` alongside the unchanged Default profile. The added profile is device-bound only, disables attestation, and enforces an allow-list containing Microsoft's Windows Hello hardware TPM, VBS hardware, and software authenticator AAGUIDs. | Dev diagnostic app through the supported v1.0 authentication-method policy PATCH and fresh readback | The Default profile remains device-bound plus synced with attestation disabled and unenforced empty restrictions; FIDO2 remains enabled, self-service remains allowed, registration remains optional, and exclusions remain empty. W73's three Conditional Access policies, both YouTrack policies, Security Defaults off, high-affinity multifactor CBA, the CBA operator, human administrative recovery, and Homer/Rachel/Kobe/admin method inventories were unchanged. No passkey was registered for proof. The retained idempotent reconciliation is `scripts/windows-hello-passkey-profile.mjs`. |
...
| Standing Entra risk-telemetry posture | Fresh native Graph reads on 2026-08-22 found Homer and Kobe `high`/`atRisk`, and Rachel and Marge `medium`/`atRisk`. Retained detections identify Homer's Medium-to-High and Kobe's High `aiCompoundAccountRisk`, plus Rachel's and Marge's Medium compounded risk; low `unlikelyTravel` sign-in detections were separately remediated by MFA or dismissed safe. Native Unified `additionalInfo` links to Defender but exposes only `UnifiedRiskSignals` for Homer, one opaque alert-type identifier plus `UnifiedRiskSignals` for Kobe, and no named contributor list for Rachel or Marge. | Dev diagnostic app with standing least-privileged `IdentityRiskyUser.Read.All` and `IdentityRiskEvent.Read.All`; dedicated CBA Global Administrator for supported Entra portal reads | Both read-only legacy Identity Protection blades have `Disabled` selected: their dormant definitions are All users, Low and above, Block access. All six enabled Conditional Access policies still have empty user-risk and sign-in-risk conditions; Security Defaults remains off and no Microsoft-managed/template policy exists. No risky user was dismissed or remediated, no user sign-in was manufactured, and no policy save was needed. This preserves risk telemetry without tenant-configured risk blocking/remediation; Microsoft service-level high-confidence blocking and limited threat-informed remediation remain platform behavior. |
| Retained Windows update baseline | On 2026-08-22 Intune accepted one Windows Update for Business ring assigned only through the exact four-member `AP2 retained managed Windows endpoints` device group. It offers Microsoft quality/security updates after a three-day deferral, uses Windows intelligent install/restart timing, gives a seven-day quality deadline plus two-day grace period, permits pre-deadline restart attempts outside active use, retains restart warnings, prevents user pause/scan overrides, and excludes driver updates from this ring. A separate assigned feature-update profile holds the scope at Windows 11 24H2 through its reported 2027-10-12 support end; all four retained devices then reported 24H2 build `26100`, so assignment offered no immediate feature upgrade. | Dev diagnostic app for exact Intune and Entra reads, bounded temporary configuration authority, creation, assignment, reconciliation, and zero-session client policy reads | Rachel, Marge, and Kobe are compliant full-MDM records. Each completed a fresh MDM cycle, logged the ring's Update CSP values as set, exposed the same effective device policy, and reached a native compliant device-context row. The effective values are `AllowAutoUpdate=6`, Microsoft updates allowed, drivers excluded, quality deferral `3`, feature/quality deadlines `14`/`7`, grace `2`, no-auto-reboot `0`, restart-warning notification level `1`, and pause/scan UI disabled. Rachel additionally retains a duplicate user-context `error` row with the same device/profile identity; its device-context `System account` row is compliant, and the sole contemporaneous client policy error was the generic `ADMXInstall/.../FakePolicy/Version` path while no Update CSP setting logged failure. Homer remains different: the exact active Entra join and Intune discovery URL are healthy, but the guest has only a type-26 discovery placeholder, no real type-6 Intune enrollment or Intune MDM device certificate, and Intune exposes only its MDE `msSense` record. The exact group assignment is ready for Homer, but application requires a supported user-credential enrollment; repeating the AAD-login reinstall would recreate a previously disproven identity path. A later current-state read reported Rachel at build `26200.9168`; the observer now accepts supported Windows 11 builds at or beyond the retained 24H2 target instead of incorrectly requiring an exact `26100` prefix. Homer, Marge, and Kobe were started with zero sessions for system-context inspection and were left to the retained scaling plan; no VM was restarted, stopped, or manually deallocated during W76. Both temporary Intune write roles are absent. The retained reconciliation/observation implementation is `scripts/windows-update-baseline.mjs`. |
| Rachel endpoint-to-passkey composed chain | Run `AP2-RACHEL-CHAIN-20260822T060500Z` began with Rachel's managed AVD endpoint launching the real AP2 company-access HTML page in Edge. MDE retained the exact Rachel-owned process, URL, and run marker; GSA independently retained an allowed successful TLS-inspected `GET`/`200` for the path with exact Rachel, device, Edge, policy, and rule attribution. Twenty-five seconds later a fresh AP2-controlled Linux/HeadlessChrome context completed Rachel CBA under the standing MFA baseline, continued in that same context to Security Info, and created one marked device-bound passkey. | Native Microsoft evidence is MDE advanced hunting, GSA traffic, Entra sign-ins, Authentication Methods/Device Registration Service audit, and Graph method inventory. AP2 controller timestamps, screenshots, egress, browser-context details, and virtual-authenticator counts are construction receipts only; see [the focused evidence map](rachel-composed-chain-evidence.md). | Rachel's audit records show self-service creation initiated by her exact object and UPN. The marked FIDO2 method was then deleted through Rachel's own portal, Graph returned to her original password-only method inventory, and worker-owned AVD session 6 was logged off. The VM remained running/available with zero sessions and no power operation. W73 plus both YouTrack policies, administrative recovery, W75 GSA/TLS state, and W76 update scope/policies remained exact. MDE returned the process event but no matching network row at capture; GSA is the native network record. Chromium emulated the device-bound authenticator, so no physical-key or Windows Hello claim is made. The retained actor/evidence paths are `scripts/homer-self-service-auth-proof.mjs`, `scripts/rachel-gsa-edge-proof.mjs`, and `scripts/rachel-chain-evidence.mjs`. |
| Deterministic retained Rachel AVD execution | Run `AP2-RACHEL-AVD-20260822T142500Z` reconciled the already-running retained VM to host `Available` with zero sessions, reconciled AVD's `Pending` session ID `-1` to exact Rachel `Active` session 6 and a 1440 by 860 remote canvas, and submitted one marker-bound Edge launch through guest-local remote keyboard input. GSA recorded transaction `74151f4f-2c2d-4f08-b3c8-50d1b875e304` about 63 seconds after Enter: Rachel, retained device `732767fb-a200-48bf-af95-817ed3906d76`, `msedge.exe`, the AP2 company-access page, `GET`, `200`, `allow`, `success`, with successful TLS interception. | CBA-authenticated Windows App/AVD for session establishment and action; ARM AVD/VM reads for readiness and cleanup; native GSA traffic for action attribution | The protected run receipt advanced from placeholder to session 6, and cleanup logged off only that ID after the native observation. Final state was VM running, host `Available`, and zero declared/listed sessions; no stop or deallocate was issued. VM start receipt, host registration, placeholder-to-active session assignment, canvas rendering, and GSA publication remain asynchronous and are reconciled by state observations rather than fixed readiness sleeps. Guest Run Command is not a readiness dependency because its serialized extension was concurrently busy. The reusable path and timing contract are in `scripts/rachel-retained-avd-execution.mjs` and [the focused execution note](rachel-retained-avd-execution.md). |
| Retained Rachel OS-backed Microsoft 365 SSO | Run `AP2-RACHEL-SSO-20260822T181500Z` established a genuine `AzureAD\\RachelGreen` CloudAP interactive session, then positively read `AzureAdJoined=YES`, a fresh `AzureAdPrt=YES`, and the WAM organizational default identity. From that session, Rachel-owned Edge processes used a brand-new, non-InPrivate data directory with no prior cookie state and reached Outlook without a login prompt. This isolates Windows PRT/WAM-backed SSO from Rachel's persistent Edge profile and uses the same Entra-joined Windows, CloudAP, PRT, WAM, and native Edge identity path expected on a physical PC; only the outer AVD session transport differs. Windows Hello is separate and was not needed for this proof. | Rachel through CBA-authenticated Windows App/AVD and her CloudAP Windows token; Dev diagnostic app only for ARM state and bounded guest inspection | The exact Edge directory, marked guest processes, staging roots, and worker-owned Rachel session are absent. Rachel and Kobe both finished running/`Available` with zero sessions; no VM power operation was issued. Kobe's healthy assigned host and Entra RDP SSO setting never progressed beyond broker `Pending` session `-1` to a numeric Windows session during bounded attempts, so it is an endpoint-specific AVD session-establishment limitation and supplies no contrary PRT or Edge SSO evidence. VM/host readiness, broker placeholder promotion, remote-canvas rendering, guest command completion, Edge redirects, and broker cleanup remain asynchronous; the retained path observes ARM session state, canvas state, guest receipts, Edge debug targets, and exact cleanup instead of treating sleeps as success. The reusable proof is `scripts/retained-avd-sso-proof.mjs`. |
| Microsoft365DSC Student baseline convergence | Microsoft365DSC `1.26.715.1` compiled a six-resource, name-bound configuration under Windows PowerShell 5.1 and its live `Test-DscConfiguration` found the selected W73 Conditional Access and W76 Windows Update resources compliant. The configuration models the retained dynamic scope-group shell without owning membership, the three enabled all-user/all-resource Conditional Access policies, the quality-update ring, and the Windows 11 24H2 feature profile. It contains no development object ID, credential, certificate, token, or generated MOF. The official resources expose name-keyed `Present` create/update and `Absent` delete paths; AP2 invokes only `Present`. | Dev diagnostic app using short-lived app-only Graph tokens on an already-running, zero-session retained runner | A bounded apply started from compliant state and remained semantically compliant. The upstream LCM nevertheless refreshed only the three Conditional Access modification timestamps, without changing IDs or modeled/unmodeled policy controls; the retained wrapper was therefore corrected to skip convergence when preflight is compliant. The W76 IDs, versions, settings, assignments, and timestamps were unchanged; no endpoint update, feature upgrade, or restart was triggered. The W77 TCM monitor stayed active and immediately matched all three policies with no changed properties. `InstallLatestWindows10OnWindows11IneligibleDevice` remains an explicitly documented create-time/independent-inspection boundary because the resource cannot update or test that immutable Graph property. Generated MOFs and temporary authority are not retained. The implementation and operational boundary are in `infra/microsoft365dsc/AP2StudentBaseline.ps1` and `docs/microsoft365dsc-student-baseline.md`. |
...
| Managed Global Secure Access baseline | On 2026-08-22 AP2 replaced the W74 proof shortcut with the current supported enterprise pattern. Intune Win32 app `Microsoft Global Secure Access client 2.31.125` (`1391526e-5197-4112-8215-188885a4c8bc`) runs as system, detects `GlobalSecureAccessClient.exe` at version `2.31.125` or later, uses Microsoft's current 2.31.125 installer, supports the documented restart return, and is required only for the exact `AP2 retained managed Windows endpoints` device group. Both enabled Microsoft 365 and Internet forwarding profiles are assigned only through the new exact four-member direct-membership group `AP2 retained managed Windows users` (`73eab301-5ce4-4079-b79a-5d4cafe6778a`); Rachel's and Kobe's proof-era direct assignments are absent. | Dev diagnostic app for supported Graph/Intune reconciliation; Rachel's CBA-authenticated My Apps session and managed Edge traffic; ARM guest reads on zero-session endpoints | Rachel retained her already-current client. Intune delivered 2.31.125 to Marge and to Kobe; Kobe's accepted install returned the documented `3010` and completed after one guarded restart. Both now expose the client and installer at 2.31.125, four running automatic GSA services, a running automatic Intune Management Extension, and Microsoft's required IPv4 preference. Homer is correctly targeted but his known MDE-only/incomplete MDM enrollment remains out of scope, so no Homer install claim is made. Fresh Rachel traffic included Microsoft 365 transaction `20260822T170142Z-17b478d8f6dn44w7hS1MNZ18us00000002d0000000001s3s` for `substrate.office.com` and successful Internet traffic with agent 2.31.125. Changing direct profile assignments to group assignment did not itself remove the My Apps entries: the entries were the assigned forwarding-profile service principals' portal exposure, not Rachel's separate TLS-inspection Conditional Access target. The two tenant-generated service principals now use supported `HideApp`, have no launch URL, and remain assignment-required; Rachel's authenticated My Apps catalog showed normal Microsoft 365 applications and no forwarding-profile tile. `HideApp` changes portal visibility only, not GSA assignment or traffic acquisition, so no unavoidable tile remains in this tenant shape. W73, W76, Defender/MDE, Intune compliance, both YouTrack policies, and administrative recovery remain intact. This worker's My Apps session was logged off; retained VMs were left running for standing idle deallocation, with no manual stop or deallocation. Reconciliation is in `scripts/global-secure-access-baseline.mjs`; managed install and endpoint observation are in `scripts/global-secure-access-client-install.ps1` and `scripts/global-secure-access-endpoint-observe.mjs`. |
| Standing narrow Global Secure Access TLS inspection | Rachel's standing Internet Access path uses one dedicated enabled security profile assigned only to her through Conditional Access. Its TLS policy defaults to bypass, retains Microsoft's enabled system bypass plus the recommended Education, Finance, Government, and Health and Medicine bypass categories, and adds one enabled inspect rule only for AP2's existing `seanewest.github.io` proof host. The customer CA is enabled and its protected root is trusted once in Rachel's local-machine root store. After one fresh Rachel AVD session and exactly one marked Edge navigation, supported Graph traffic transaction `6ca48e8b-0a1f-442c-8949-104c87a099b7` independently retained Rachel, her managed device, agent 2.31.125, `msedge.exe`, `seanewest.github.io:443`, `allow`, `success`, and East US 2 plus materially richer HTTPS details: `https://seanewest.github.io/ap2/company-access/`, method `GET`, response `404`, and TLS `intercepted` / `success` tied to the exact narrow policy and rule. | Rachel through Edge on `ap2fastrachel`; Dev diagnostic app for supported Graph/ARM reconciliation and backend observation | The query-string proof marker was not retained in `destinationUrl`, but the distinguishable HTTPS path, method, and response were. No content-blocking rule exists. The CA, endpoint trust, default bypass, system and recommended bypasses, narrow TLS rule, single-policy security profile, and Rachel-only Conditional Access assignment intentionally remain standing. Rachel now acquires the Internet forwarding profile through the retained managed-user group; TLS inspection remains separately Rachel-only and was not broadened for baseline uniformity. The five W73/YouTrack policies remain enabled. Protected CA material is included in the existing encrypted development-runtime vault snapshot, whose 48-file restore set passed hash validation and an ARM subscription read. Reconciliation and traffic observation are in `scripts/rachel-gsa-standing.mjs`; the bounded actor request is in `scripts/rachel-gsa-edge-proof.mjs`. |
