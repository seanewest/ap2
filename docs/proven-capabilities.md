...
| SharePoint content tampering and recovery | The Dev diagnostic app created one uniquely named, unshared harmless file and overwrote it once with different harmless content. One capped, unpaged version read returned exactly the expected original and overwritten versions. | Defender recovery restored the unambiguous original version and proved the exact original bytes before conditionally deleting the file. Exact ID and path reads then returned `404`. A later distinct fixed detector reached Microsoft Graph v1 Purview Audit Search app-only without changing authority. | Overwrite and version restoration were unconditional, so this proves only the fresh, canary-owned file contract. One historical keyword audit query succeeded with zero records; one reviewed exact-path correction remained `notStarted` through three capped reads. Operation-level producer attribution therefore remains `licensing-or-latency-blocked`, not proven. The product minimum is `AuditLogsQuery-SharePoint.Read.All`; the retained diagnostic detector currently has a broader audit role. See [the Purview operation-audit contract](purview-audit-contract.md). |
