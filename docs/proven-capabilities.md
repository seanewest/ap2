...
| Standing MFA enforcement baseline | A fresh read-only audit of all 16 enabled Entra-Suite simulated users found Security Defaults disabled and legacy per-user MFA disabled for all 16. All 16 had a password method; only two had any second method, Microsoft Authenticator registration, or MFA-capable/registered status. The tenant had two enabled Conditional Access policies, both narrow YouTrack browser session controls with no grant control, MFA requirement, authentication strength, risk, location, device, or general Microsoft 365 target. Recent sign-in evidence independently corroborated the policy interpretation: all 384 successful interactive records in the bounded observed subset were classified single-factor, none required MFA, and three records for two users showed exactly one successful `Password` / `Correct password` step. Each password event was followed within 30 minutes by successful single-factor access to Microsoft Graph and Office 365 Shell, with Conditional Access not applied and no MFA record. Password-only compromise is therefore an authentic standing premise for ordinary Microsoft 365 access. | Dev diagnostic app for policy, registration, method, and sign-in-log reads; existing simulated-user sign-ins supplied the corroborating records | Read-only; no sign-in was manufactured. Only five of 16 users had recent interactive records, five per-user reads reached the 100-record cap, and current risky-user state was unavailable to the Dev app (`403`). Those limits do not create an enforcement ambiguity: current Security Defaults is off, no current Conditional Access policy has an MFA/authentication-strength or risk grant, and all observed successful records were single-factor. Microsoft-mandated MFA for Azure and Microsoft admin portals remains a separate service-scoped boundary and does not protect ordinary Microsoft 365 resources. |
...
| --- | --- | --- | --- |
| Benign enrollment to a distinct authenticated session | In run `AP2-RACHEL-FIRSTLEG-20260821T140100Z`, Rachel authenticated to her assigned healthy AVD desktop through genuine Windows App X.509 authentication. In guest session 2, `AzureAD\RachelGreen` opened a fresh Edge InPrivate window to an AP2-owned localhost company-access page. Edge recorded a visit at `14:06:05Z`; one trusted `Continue with Microsoft` click at `14:06:25Z` navigated to the public AP2 SPA. Microsoft first recorded the expected unavailable-certificate attempt, then accepted Edge's existing Rachel-owned saved password through its normal secure autofill without exposing or transferring the credential. | Protected visual evidence shows the InPrivate AP2 SPA visibly `Signed in as Rachel Green` with the exact Rachel UPN. Entra later recorded `After Party Exploratory` / Microsoft Graph success at `14:09:18Z`; its preceding expected keep-signed-in interrupt recorded `Password`, `Correct password`, and `succeeded: true`. The AP2 UI simultaneously reported that this tenant had no usable API connection and disabled actions, so the composition stopped without post-sign-in activity. AVD independently attributed the active desktop to Rachel, and a later guest read attributed both Explorer and the top-level Edge process to Rachel in session 2. The enrollment receipt bound the exact run, Edge user agent, and trusted click. Rachel's authentication-method inventory remained password-only. MDE remained onboarded and healthy with no matching alert; the endpoint had no GSA service, so no GSA event is claimed. The exact listener task/root and Rachel session were removed, and the VM was returned deallocated with host `Shutdown` and zero sessions. | This convincingly represents the first-leg ATO condition as a new AP2-authenticated InPrivate session under the simulated user's identity; it does not reproduce credential theft, token replay, MFA bypass, attacker-controlled infrastructure, or an adversary obtaining access. The enrollment page, endpoint, browser control, saved credential, and AP2 site are all owned lab assets. Edge InPrivate supplied isolation and disposable browser state, not anonymity. The AP2 password-success record arrived after cleanup in a bounded propagation follow-up; the earlier failed AP2 X.509 attempt and successful Windows App/AVD X.509 trail are retained as part of the coherent chronology. No payload, persistence, authentication-method registration, policy change, or post-ATO action occurred. |
