...
| Three-VM AVD lab substrate | One fresh run provisioned a private Windows 11 Enterprise personal AVD host and two private Canonical Ubuntu 24.04 auxiliary nodes in parallel behind one shared VNet/NAT/public-IP egress boundary. The auxiliaries reached ARM and VM-Agent readiness in 43 and 47 seconds; the AVD host became available and direct-assigned in 11 minutes 40 seconds; the complete ARM deployment finished in 16 minutes 25 seconds; and deployment-time Intune compliance plus local and API Defender onboarding completed by 19 minutes 8 seconds. Windows Run Command retrieved both fixed marker health endpoints over private addresses without a learner session. | Dev diagnostic app for Azure, Intune, Defender, and fixed Run Command probes; independent administrator for the bounded temporary-permission lifecycle | The Linux nodes used Trusted Launch, cloud-init, VM Agent, and Run Command rather than false Intune or Defender client claims. Live Defender offboarding returned local state to `0`; all exact run resources, endpoint objects, temporary permissions, expiry automation, and sensitive artifacts are absent. The conservative four-hour public-price projection was USD 4.21490411, with a full extra provisioning hour at USD 4.59363014—below the USD 5 target and USD 10 lane ceiling. Ordinary provider, audit, deleted-group, Intune-report, and Defender machine-history residue can remain. |
...
The source-backed migrations are the already-proven controlled Teams
missed-call observation and application reconnaissance. Their product cards
are read-only and perform no tenant operation:
...
| SharePoint content tampering and recovery | The Dev diagnostic app created one uniquely named, unshared harmless file and overwrote it once with different harmless content. One capped, unpaged version read returned exactly the expected original and overwritten versions. | Defender recovery restored the unambiguous original version and proved the exact original bytes before conditionally deleting the file. Exact ID and path reads then returned `404`. | Overwrite and version restoration were unconditional, so this proves only the fresh, canary-owned file contract. Operation-level Unified Audit Log correlation remains unproven because its supported app-only path requires separate Purview RBAC; existing Graph audit access and Exchange Administrator are insufficient. |
