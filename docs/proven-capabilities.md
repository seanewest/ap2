...
| Endpoint DLP paste enforcement and destination limitation | After Windows device monitoring was enabled and Sense recorded receipt and processing of the scoped Devices policy, Edge 151 blocked Microsoft's synthetic Credit Card Number sample in an unsent `chatgpt.com` prompt. Activity Explorer later recorded `Pasted to browser` at `2026-08-11T08:42:07Z` for Kobe, `ap2flakobe`, `msedge.exe`, target `chatgpt.com`, and enforcement mode `Block`. Destination differentiation is not yet proven: with only a second rule whose overall paste action was `Audit` and whose sole custom `chatgpt.com` group override was `Block`, the same paste into an unsaved YouTrack tab-name field did not complete and remained under evaluation after 22 seconds; no YouTrack paste event appeared. The endpoint received that isolated policy state before the YouTrack attempt. | Kobe on `ap2flakobe` | Both temporary policies and the custom domain group are absent; Windows device monitoring is back off, the synthetic file and user session are absent, and the VM is deallocated. Policy delivery took about eight minutes in this run. Activity Explorer had not surfaced the event after roughly 12 minutes but had surfaced it by 44 minutes. |
...
| YouTrack SCIM deactivation persistence | Before deactivation, Kobe's permanent token read `DEMO-13` with HTTP `200` and configured a harmless custom app marker. After Entra group removal and SCIM deactivation, YouTrack reported Kobe banned and retained the token record, but the same token read returned `403`; an admin-authenticated app status call still returned `200` with Kobe's marker and configuring identity. After group restoration and SCIM reactivation, the unchanged token again read `DEMO-13` with `200`, and the app marker was unchanged. This proves token persistence without authorization while banned, plus app configuration and execution persistence; it does not prove post-deactivation delivery by a user-authorized outbound webhook. | Kobe for token and app configuration; YouTrack administrator for state observation | Kobe is restored in `AP2 YouTrack Users` and unbanned. Both SCIM jobs are active and last succeeded; the temporary token and app are absent. |
