...
- GitHub Enterprise Cloud with Enterprise Managed Users proved Entra OIDC SSO, Entra SCIM managed-user lifecycle, managed enterprise administration, and attributable organization activity in the `ap2-v2` enterprise. Sean later manually completed the Microsoft Defender for Cloud Apps GitHub API App Connector flow after agent-driven browser attempts had encountered a disabled OAuth-consent control. The successful human flow proves that observation was not a platform-level EMU/connector blocker; the reason the agent browser saw the disabled control remains unknown, and connector telemetry is a separate result to verify.
