...
| Authentication-method posture | After the new read-only role reached a fresh token, one app-only execution read the four fixed simulated users without paging or user sign-in. The aggregate contained six registered methods: four password and two Microsoft Authenticator registrations, with no other method category observed. Method details were not retained. Registration does not prove MFA policy or enforcement. | Dev diagnostic app | Read-only; the `UserAuthenticationMethod.Read.All` assignment and its private cleanup identity are retained |
