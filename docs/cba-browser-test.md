...
tenant ID, clicks the product sign-out button, completes Microsoft's logout
redirect, and verifies the signed-out state again after a reload. Browser
output defaults to `/tmp/ap2-playwright-cba` and contains no reusable storage
state.
